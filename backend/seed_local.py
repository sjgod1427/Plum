"""
Seed the LOCAL SQLite DB with the 10 known test results from test_results_full_pipeline.md.
No API calls — instant.

Usage (with backend stopped):
    cd backend && uv run python seed_local.py
"""
...
def main():
    ...
    with Session(engine) as session:
        for row in SEED_DATA:
            ...
            session.add(Claim(
                ...
                cashless_request=row["cashless_request"], ytd_claimed_amount=0.0,
    ...
    print(f"\nSeeded {len(SEED_DATA)} test cases into local claims.db")
